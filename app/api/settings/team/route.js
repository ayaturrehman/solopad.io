...
export async function POST(req) {
  ...
  const fromEmail = process.env.RESEND_FROM_EMAIL || process.env.FROM_EMAIL || "noreply@portalkit.app";
  ...
  try {
    ...
    await resend.emails.send({
      from: fromEmail,
      to: email,
      subject: `${session.user.name} invited you to collaborate in PortalKit`,
      html: `
        <p>Hi ${name},</p>
        <p><strong>${session.user.name}</strong> invited you to collaborate in PortalKit.</p>
