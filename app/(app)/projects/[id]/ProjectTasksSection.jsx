...
export default function ProjectTasksSection({ project, tasks: initialTasks, teamMembers }) {
  ...
  async function handleAddTask(e) {
    ...
    if (res.ok) {
      ...
      const nextTask = { ...task, status: formData.status || task.status };
      setTasks((prev) => [nextTask, ...prev]);
      resetForm();
      setShowForm(false);
      if (nextTask.description || nextTask.subtasks?.length) {
        setExpandedTaskIds((prev) => new Set(prev).add(nextTask.id));
      }
    }
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {hasDetails && editingId !== task.id && (
                  <button type="button" onClick={() => toggleExpanded(task.id)} className="rounded p-1 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700">
    ...
    <div className="space-y-4">
      ...
      <div className="rounded border border-zinc-200 bg-white">
        {sortedTasks.length === 0 ? (
          <p className="px-6 py-12 text-center text-sm text-zinc-400">No tasks in this project yet. Add the first one above.</p>
        ) : (
