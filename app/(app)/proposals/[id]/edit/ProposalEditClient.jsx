...
export default function ProposalEditClient({ proposal, projects }) {
  ...
  const [validUntil, setValidUntil] = useState(
    proposal.validUntil ? new Date(proposal.validUntil).toISOString().split("T")[0] : ""
  );
  const [currency, setCurrency] = useState(proposal.currency || "USD");
  const [intro, setIntro] = useState(proposal.intro || "");
  const [sections, setSections] = useState(() => {
    const parsed = parseJsonArray(proposal.sections);
    return parsed.length ? parsed : [{ heading: "", body: "" }];
  });
  const [pricing, setPricing] = useState(() => {
    const parsed = parseJsonArray(proposal.pricing);
    return parsed.length ? parsed : [{ description: "", amount: "" }];
  });
  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-3">
        <Link href={`/proposals/${proposal.id}`} className="inline-flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-900">
