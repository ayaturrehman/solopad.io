  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // Load team member data for permissions
  ...
  callbacks: {
    jwt({ token, user, trigger, session }) {
      if (user) {
       ...
       if (trigger === "update" && session?.user) {
        ...
        if (session.user.teamRole !== undefined) token.teamRole = session.user.teamRole;
        if (session.user.permissions !== undefined) token.permissions = session.user.permissions;
    ...
    session({ session, token }) {
      if (token) {
        ...
        session.user.teamRole = token.teamRole || token.role;
