...
export async function getTenantFilter(session) {
  ...
  // Fast path: if businessId is already in the session JWT, skip the DB lookup
  if (session.user.businessId) {
    return { businessId: session.user.businessId };
  }

  // Check if session explicitly has businessId set to null (JWT populated but solo user)
  if (session.user.businessId === null) {
    return { userId: session.user.id };
  }

  // Legacy fallback: session JWT doesn't include businessId yet (old sessions)
